...
class PDF extends PDF_HTML {
	...
	public function __construct($project, $kampen) {
		parent::FPDF();
		$this->project = $project;
		$this->jaar = $project->jaar;
		$this->kampen = $kampen;
	...
	function Header()
	{
		...
		if ($this->kampNaam == '') {
			$this->Cell(0, 10, 'Overzicht gegevens HIT '. $this->plaats .' '.$this->jaar, 'B', 0, 'C');
		} else {
			$this->Cell(0, 10, $this->c('HIT '. $this->plaats .' '. $this->jaar ." - ". $this->kampNaam), 'B', 0, 'C');
		}
		$this->Ln(20);
	}
 ...
 * HitProject PDF View.
 */
class KampInfoViewHitProject extends JViewLegacy {
